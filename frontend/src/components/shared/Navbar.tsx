        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            ...
            <div className="ml-2 leading-tight">
              <div className={`text-xl font-semibold ${isTeal ? 'text-white' : 'text-gray-900'}`}>
                {brandTitle}
              </div>
              {brandSubtitle && (
                <div className={`text-xs ${isTeal ? 'text-teal-100' : 'text-gray-500'}`}>
                  {brandSubtitle}
                </div>
              )}
            </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            <button
              ...
              className={`font-medium ${activeSection === 'home' ? activeLinkClass : baseLinkClass}`}
            >
              Home
            </button>
            <button
              onClick={onAboutClick ?? (() => navigate('/home'))}
              className={`font-medium ${activeSection === 'about' ? activeLinkClass : baseLinkClass}`}
            >
              À propos
            </button>
            <button
              onClick={onServicesClick ?? (() => navigate('/home'))}
              className={`font-medium ${activeSection === 'services' ? activeLinkClass : baseLinkClass}`}
            >
              Services
            </button>
            <button
              onClick={onWhyUsClick ?? (() => navigate('/home'))}
              className={`font-medium ${activeSection === 'whyUs' ? activeLinkClass : baseLinkClass}`}
            >
              Pourquoi nous
            </button>

            {/* Blog et Contact : scroll en haut */}
            <button onClick={() => navigateTop('/blog')} className={`font-medium ${baseLinkClass}`}>
              Blog
            </button>
            <button onClick={() => navigateTop('/contactUs')} className={`font-medium ${baseLinkClass}`}>
              Contact
            </button>
          </div>
